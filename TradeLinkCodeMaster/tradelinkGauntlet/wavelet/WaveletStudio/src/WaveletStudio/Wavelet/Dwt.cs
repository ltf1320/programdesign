...
namespace WaveletStudio.Wavelet
{
    ...
    public static class DWT
    {
        ...
        /// <returns></returns>
        public static List<DecompositionLevel> ExecuteDWT(Signal signal, MotherWavelet motherWavelet, int level, SignalExtension.ExtensionMode extensionMode = SignalExtension.ExtensionMode.SymmetricHalfPoint, ConvolutionModeEnum convolutionMode = ConvolutionModeEnum.ManagedFFT, bool returnOnlyValid = true)
        {
            ...
            for (var i = 1; i <= level; i++)
            {
                ...
                approximation = WaveMath.Convolve(convolutionMode, approximation, motherWavelet.Filters.DecompositionLowPassFilter, returnOnlyValid);
                approximation = WaveMath.DownSample(approximation);

                details = WaveMath.Convolve(convolutionMode, details, motherWavelet.Filters.DecompositionHighPassFilter, returnOnlyValid);
        ...
        /// <param name="convolutionMode">Defines what convolution function should be used</param>
        /// <returns></returns>
        public static double[] ExecuteIDWT(List<DecompositionLevel> decompositionLevels, MotherWavelet motherWavelet, int level = 0, ConvolutionModeEnum convolutionMode = ConvolutionModeEnum.ManagedFFT, bool returnOnlyValid=true)
        {
            ...
            for (var i = level - 1; i >= 0; i--)
            {
                ...
                approximation = WaveMath.Convolve(convolutionMode, approximation, motherWavelet.Filters.ReconstructionLowPassFilter, returnOnlyValid, -1);

                details = WaveMath.UpSample(details);
                details = WaveMath.Convolve(convolutionMode, details, motherWavelet.Filters.ReconstructionHighPassFilter, returnOnlyValid, -1);
                ...
                if (i <= 0) //少了一次循环
                {
                    approximation = SignalExtension.Deextend(approximation, decompositionLevels[0].Signal.SamplesCount);
                    continue;
                }
