...
namespace Responses
{
    public class SMAResponse : ResponseTemplate
    {
        ...
        Boolean useSMAFromATSGlobalIndicator = true;
        [Description("UseSMAFromATSGlobalIndicator")]
        public Boolean UseSMAFromATSGlobalIndicator { get { return useSMAFromATSGlobalIndicator; } set { useSMAFromATSGlobalIndicator = value; } }
        
        SMA SMAFromATSGlobalIndicator = null;
        ...
        public override void Reset()
        {
            ...
            SMAFromATSGlobalIndicator = new SMA();
            SMAFromATSGlobalIndicator.Param = SMAInputParamTmp;
            SMAFromATSGlobalIndicator.Initializatin();
        }
        ...
        void blt_GotNewBar(string symbol, int interval)
        {
            ...
            Bar _lastBar = _myBars[_myBars.Count - 1];
            if (UseSMAFromATSGlobalIndicator)
            { 
                SMAFromATSGlobalIndicator.UpdateValue(_lastBar.Close);
                SMA = SMAFromATSGlobalIndicator.GetSignal();
            }
            else
            {
                SMA = Calc.Avg(Calc.EndSlice(blt[symbol].Open(), _barsback));
            }
            ...
            sendindicators(new string[] { time.ToString(), SMA.ToString("F5", System.Globalization.CultureInfo.InvariantCulture), _lastBar.Close.ToString("F5", System.Globalization.CultureInfo.InvariantCulture) });
