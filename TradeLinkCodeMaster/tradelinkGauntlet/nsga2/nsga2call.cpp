...
int angle2;
using namespace std;
string MsgLog("calling nsga2..");

FILE *gp;
int read_inputParam_from_file(char *file_name);
int read_inputParam();
...
int read_inputParam_from_file(char *file_name)
{
    ...
    // read he population size (a multiple of 4)
	readline(fp);
    sscanf(line,"%d",&popsize);
    if (popsize<4 || (popsize%4)!= 0)
    {
        printf("\n population size read is : %d",popsize);
        printf("\n Wrong population size entered, hence exiting \n");
   ...
   //read the number of generations	
    ...
    //read the number of real variables
	readline(fp);
    sscanf(line,"%d",&nreal);
    if (nreal<0)
    {
    ...
    if (nreal != 0)
    {
        ...
        for (i=0; i<nreal; i++)
        {
            //read Enter the lower limit of real variable %d 
			readline(fp);
            sscanf(line,"%lf",&min_realvar[i]);
            //read the upper limit of real variable
			p = line;
			SKIP_TARGET
            sscanf (p,"%lf",&max_realvar[i]);
            if (max_realvar[i] <= min_realvar[i])
            {
                printf("\n Wrong limits entered for the min and max bounds of real variable, hence exiting \n");
                return -1;
        ...
        //read Enter the probability of crossover of real variable (0.6-1.0)
		readline(fp);
        sscanf (line,"%lf",&pcross_real);
        if (pcross_real<0.0 || pcross_real>1.0)
        {
            printf("\n Probability of crossover entered is : %e",pcross_real);
            printf("\n Entered value of probability of crossover of real variables is out of bounds, hence exiting \n");
        ...
        //read the probablity of mutation of real variables (1/nreal)
		readline(fp);
        sscanf (line,"%lf",&pmut_real);
        if (pmut_real<0.0 || pmut_real>1.0)
        {
            printf("\n Probability of mutation entered is : %e",pmut_real);
            printf("\n Entered value of probability of mutation of real variables is out of bounds, hence exiting \n");
        ...
        //read the value of distribution index for crossover (5-20)
		readline(fp);
        sscanf (line, "%lf",&eta_c);
        if (eta_c<=0)
        {
            printf("\n The value entered is : %e",eta_c);
            printf("\n Wrong value of distribution index for crossover entered, hence exiting \n");
        ...
        //read Enter the value of distribution index for mutation (5-50):
		readline(fp);
		sscanf (line,"%lf",&eta_m);
        if (eta_m<=0)
        {
            printf("\n The value entered is : %e",eta_m);
            printf("\n Wrong value of distribution index for mutation entered, hence exiting \n");
    ...
    if (nbin != 0)
    {
        ...
        for (i=0; i<nbin; i++)
		{
            // read the number of bits for (i+1)'th binary variable
			readline(fp);
            sscanf (line,"%d",&nbits[i]);
            if (nbits[i] < 1)
            {
                printf("\n Wrong number of bits for binary variable entered, hence exiting");
                return -1;
			...
			p = line;
            //read  the lower limit of (i+1)'th binary variable
			SKIP_TARGET
            sscanf (line,"%lf",&min_binvar[i]);
            // read the upper limit of (i+1)'th binary variable
			SKIP_TARGET
            sscanf (line,"%lf",&max_binvar[i]);
            if (max_binvar[i] <= min_binvar[i])
            {
                printf("\n Wrong limits entered for the min and max bounds of binary variable entered, hence exiting \n");
                return -1;
        ...
        //read  the probability of crossover of binary variable (0.6-1.0)
		readline(fp);
        sscanf (line,"%lf",&pcross_bin);
        if (pcross_bin<0.0 || pcross_bin>1.0)
        {
            printf("\n Probability of crossover entered is : %e",pcross_bin);
            printf("\n Entered value of probability of crossover of binary variables is out of bounds, hence exiting \n");
        ...
        //read the probability of mutation of binary variables (1/nbits)
		readline(fp);
        sscanf (line,"%lf",&pmut_bin);
        if (pmut_bin<0.0 || pmut_bin>1.0)
        {
            printf("\n Probability of mutation entered is : %e",pmut_bin);
            printf("\n Entered value of probability  of mutation of binary variables is out of bounds, hence exiting \n");
    ...
    choice=0;
    // read whether use gnuplot to display the results realtime (0 for NO) (1 for yes) 
	readline(fp);
    ...
    if (choice==1)
    {
		...
		*/
        if (nobj==2)
        {
            //read the objective for X axis display 
			readline(fp);
            sscanf(line,"%d",&obj1);
            if (obj1<1 || obj1>nobj)
            {
            ...
            //read the objective for Y axis display
			readline(fp);
            sscanf(line,"%d",&obj2);
            if (obj2<1 || obj2>nobj)
            {
        ...
        {
            //while #obj > 2, read 2D display or a 3D display 2 for 2D and 3 for 3D
			readline(fp);
            sscanf(line,"%d",&choice);
            if (choice!=2 && choice!=3)
            {
            ...
            if (choice==2)
            {
                ...
                //read objective for Y axis display 
				readline(fp);
                sscanf(line,"%d",&obj2);
                if (obj2<1 || obj2>nobj)
                {
            ...
            else
            {
                ...
                //read the objective for Z axis display 
				readline(fp);
                sscanf(line,"%d",&obj3);
                if (obj3<1 || obj3>nobj)
                {
                ...
                // You have chosen 3D display, hence location of eye required,
                // read the first angle (an integer in the range 0-180) (if not known, enter 60)
				readline(fp);
                sscanf(line,"%d",&angle1);
                if (angle1<0 || angle1>180)
                {
                    printf("\n Wrong value for first angle entered, hence exiting \n");
                    return -1;
                }
                //rad the second angle (an integer in the range 0-360) (if not known, enter 30)
                sscanf(line,"%d",&angle2);
                if (angle2<0 || angle2>360)
                {
                    printf("\n Wrong value for second angle entered, hence exiting \n");
                    return -1;
...
void maintestwithmsg(int argc, char **argv, LPSTR outMsg)
{
	maintest(argc, argv);
