...
namespace ODataWebTests
{
    ...
    public class ODataWebSessionTest
    {
        ...
        private void CreateSession()
        {
            ...
            Uri sessionedUri = new Uri("http://services.odata.org/(S(" + sessionId + "))/OData/OData.svc/", UriKind.Absolute);
        ...
        [TestMethod]
        public void InvokeServiceOp()
        {
            //Product
            var query = ctx.CreateQuery<Product>("GetProductsByRating").Expand("Category").AddQueryOption("rating", "3");
            Assert.AreEqual(8, query.Count());

            var p = query.Skip(3).Take(1).FirstOrDefault();
            Assert.IsNotNull(p);
