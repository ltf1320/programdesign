...
void RecvProcess(LPVOID pParam)
{
	...
	//保存聊天记录
		...
		//播放声音
		sndPlaySound(L"recv.wav",SND_ASYNC);
...
BOOL CLANTalkDlg::OnInitDialog()
{
	...
	{
		MessageBox(L"Socket 创建失败",L"错误",MB_OK);
	}
...
void CLANTalkDlg::OnClose() 
{
	if (m_bShutdown)
	{
		...
		m_szInputMsg = "聊天结束";
		sendto(m_sendSock, reinterpret_cast<char*>(m_szInputMsg.GetBuffer(m_szInputMsg.GetLength())),
...
void CLANTalkDlg::OnOK() 
{
	...
	{
		MessageBox(L"不能发送空消息",L"Error");
	}

	if (m_szInputMsg.GetLength() > 0)  //如果输入的消息不为空，发送
	{
		if (sendto(m_sendSock, reinterpret_cast<char*>(m_szInputMsg.GetBuffer(m_szInputMsg.GetLength())),
			m_szInputMsg.GetLength(), 0, reinterpret_cast<sockaddr*>(&m_addrto),
			sizeof(m_addrto)) == SOCKET_ERROR)
			MessageBox(L"发送报文失败",L"Error");
	}
...
void CLANTalkDlg::OnBtnIp() 
{
	...
	CString str;
	str.Format(L"你目前的信息发送对象为：%s",m_szAddr);
