...
class aFrame extends JFrame implements ActionListener,MouseMotionListener,MouseListener,Serializable{
	...
	public aFrame(){
        ...
        SaveBtn=factory.CreateButton("save");
        LoadBtn=factory.CreateButton("load");
        p.add(BR);
	    p.add(BL);
	    p.add(BS);
	    p.add(M);
	...
	public void save()
	{
		...
		if(fileName!=null)
		{
			try{
				...
				out.writeInt(iterator.getsize());
				for(int i=0;i<iterator.getsize();i++)
				{
		//			out.writeInt(iterator.get(i).getType());
					out.writeObject(iterator.get(i));
				}
			}
			catch(Exception e)
			{
	...
	public void load()
	{
		...
		if(fileName!=null)
		{
			try{
				...
				{
					shap sp;
					sp=(shap)in.readObject();
					iterator.addElement(sp);
				}
			}
			catch(Exception e)
			{
