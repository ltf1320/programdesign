...
public class DoubleStringCal {
	...
	public static double calculate(String arg) throws StringCalException
	{
		arg=dispose(arg);
		...
		while(in.hasNext())
		{
			...
			if(nowPri<lastPri)
			{
/*				if(!in.hasNext())  throw new StringCalException("Error 0001:expretion is incorrect");
				detmp=in.nextDouble();
				numStack[numTop-1]=calculate(numStack[numTop-1],detmp,chtmp);
				if(!in.hasNext()) break;
				chtmp=(char) in.nextInt();
				if(chTop==0) lastPri=braPri;
				else lastPri=getPri(chStack[chTop-1]);
*/
	...
	public static double calculate(double a,double b,char s) throws StringCalException
	{
		switch(s)
		{
			case '+':return a+b;
