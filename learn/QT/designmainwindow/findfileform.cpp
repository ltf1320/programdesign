...
void CFindFileForm::openFile()
{
    QList<QTableWidgetItem*> items=resultTableWidget->selectedItems();
    if(items.count())
    {
        QTableWidgetItem *item=items[0];
        qDebug()<<item->row()<<" "<<item->column();
        item=resultTableWidget->item(item->row(),4);
        qDebug()<<item->row()<<" "<<item->column();
        if(mainWindow&&item)
            mainWindow->OpenFile(item-> text());
    }
}

void CFindFileForm::setMainWindow(CMainWindow *mainWin)
{
    mainWindow=mainWin;
}
